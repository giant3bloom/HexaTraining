...
db.courses.find({price: {
    $gt: 8000,
    $lt: 18000
}})
db.courses.find({level: {
    $in: ['Beginner', 'Intermediate']
}})
db.enrollments.find({'progress.completion_percent': {
    $gte: 80
}})
db.enrollments.find({
    'payment.status': {$ne: 'Success'}
})
db.learners.find({city: {
    $in: ['Hyderabad', 'Bangalore', 'Pune']
}})
db.courses.find({category: {
    $ne: 'Cloud'
}})
...
db.enrollments.aggregate([
    {
        $group: {
            _id: '$course_id',
            avg_completion: {$avg: '$progress.completion_percent'}
        }
    }
])
db.enrollments.aggregate([
...
db.learners.aggregate([
    {
        ...
        $project: {
            _id: 0,
            enrollment_id: '$enrollment_details.enrollment_id',
            name: 1,
            city: 1,
            course_id: '$enrollment_details.course_id',
            status: '$enrollment_details.status'
        }
    }
])

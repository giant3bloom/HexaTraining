...
db.customers.find()
db.restaurants.find()
db.restaurants.find({}, {name: 1, city: 1, _id: 0})
...
db.orders.find({'payment.status': {$nin: ['Success']}})
...
db.customers.updateOne({customer_id: 1}, {$set: {membership: 'Platinm'}})
...
db.orders.updateMany({}, {$set: {active: true}})
db.orders.updateMany({}, {$unset: {active: ''}})
db.orders.updateOne(
    {order_id: 1006}, 
    {
        $push: {
